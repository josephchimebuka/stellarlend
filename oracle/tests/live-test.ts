...
async function testLive() {
  ...
  const aggregator = createAggregator([coingecko, binance], validator, cache, { minSources: 1 });
  ...
  try {
    ...
    for (const [asset, data] of prices) {
      ...
      console.log(
        `  ${asset}: $${priceNum.toFixed(asset === 'XLM' ? 4 : 2)} (confidence: ${data.confidence.toFixed(0)}%, sources: ${data.sources.length})`
      );
    }
  } catch (err) {
